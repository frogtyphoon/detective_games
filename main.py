...
for i in range(900, 1, -1):  # 900сек = 15 мин
...
sleep(3)
loop_m('Убит мужчина, 40 лет. Тело найдено в загородном доме района Алексеева Предположительна'
...
loop_m_photo('AgACAgIAAxkDAAIBcl8QPvfIUXWjneqA4r78pJjMLBdBAAJlrjEbPNiBSDGYrVxwYlWZ5k71lC4AAwEAAwIAA3kAA-T_AQABGgQ')
...
if time_relationships == 1:
    ...
    if wait() == 1:
        ...
        loop_m_photo('AgACAgIAAxkDAAIBsV8QTNpMgQgJVhLl2hTFU_7lYbKJAAJ-rjEbPNiBSJFGvLUqu6ZgyQABUZEuAAMBAAMCAAN5AANn9gUAARoE')
        sleep(10)
...
sleep(3)
loop_m('Убита юная девушка 19 лет в районе Весны города Красноярска. '
...
# считает улики и видёт подсчёт времени
for i in range(600, 1, -1):  # 600 = 10 мин
    if i == 600 or i == 300 or i == 60:  # оповещение для админа
        loop_m_admin(str(i // 60) + 'мин')
    f = open('text.txt', 'r+', encoding='utf8')
    text = [str(i) for i in f.read().split('|')]
...
edit_main('Bot__police', 19, 'Всё, пора уходит. Сам понимаешь почему...\nНенавижу их. Всегда мешают работать!')
relationships_Bot__police += wait()
...
# досье
loop_m('Досье для пятого убийства', None, config.TOKEN_CRIMINALIST)
...
for i in suspect:
    ...
    if i == 'Подруга, Инна':
        ...
        loop_m('Совкин Станислав Максимович')
...
sleep(3)
loop_m('Это всё. Сделай правильный выбор')
...
# концовка
loop_m('Напиши, кто по твоему убийца?', None, config.TOKEN_POLICE)
