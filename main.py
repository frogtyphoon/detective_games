...
loop_m('Чёрт, Кристина, ты где пропала?')  # функция, позволяющая отправлять сообщения
loop_m('Уже неделя прошла... Где ты пропадаешь???')
loop_m('Если не отвечаешь через неделю, то ты уволена!')
...
if relationships_Bot__police >= 0:
    ...
    if time_relationships < 0:
        edit_main('Bot__police', 2, 'Совсем страх потеряла?')
        if wait() < 0:
            ...
            edit_main('Bot__police', 3, 'Чтобы через 15 мин была там, ок?!')
            relationships_Bot__police += wait()
    else:
        loop_m('У нас неподалеку совершилось самоубийство. Тебе стоит выехать и разобраться.'
               ' Возможно, здесь что-то не чисто. Это серьёзное дело, будь готова ко всему')
        sleep(2)
        ...
        sleep(1)
        edit_main('Bot__police', 3, 'Всё уяснила, Кристина?')
        relationships_Bot__police += wait()

# Вторая развязка (4)
else:
    edit_main('Bot__police', 4, 'Лучше бы заткнулася и послушала меня, пока не стало хуже!')
    time_relationships = wait()
    relationships_Bot__police += time_relationships
    if time_relationships < 0:
        time_talk_answer = True
...
sleep(300)  # пока 10, должно быть 300сек = 5мин
if time_talk_answer:
...
for _ in range(900):  # 900сек = 15 мин
    f = open('text.txt', 'r+', encoding='utf8')
    text = [str(i) for i in f.read().split('|')]
    f.close()
    if text[0] == 'main':
...
# выбор подзреваемо
loop_m('Мы узнали много нового и перед нами есть пару подозррительных личностей. '
...
# подсказка при хороших отноениях с криминалистом
if relationships_Bot__criminalist > 0:
...
if wait() == 0:
    sleep(3)
    loop_m('Парень задержан. Его Алиби - "Я пришел к ней в восьмом часу, может, в 7:10. Ушел от нее в 7:40.'
    ...
    sleep(3)
    loop_m('Подруга задержана. Её Алиби - "Я виделась с ней вчера вечером, все было хорошо. В 9 утра я была на другом'
...
if wait() == 1:
    loop_m('Её алиби:  В это время я была на кладбище, мне было тяжело, я волновалась, поэтому'
    ...
    loop_m('Её алиби: Я была на приёме у стоматолога, с 10:40 до 11:50 включительно.»')
...
sleep(5)
loop_m('Убита женщина 39 лет, тело найдено возле дома, в районе Н. Предположительная причина смерти: случайно выпала из'
       ' окна во время мойки окна. Перелом 80% костей, травмы, несовместимые с жизнью.')
sleep(2)
loop_m('Время смерти: 07.07.2020, 14:00')
sleep(4)
loop_m('Без вяских разговоров выдвигайся по адресу "АДРЕС". У нас не так много времени, всего 20 мин')

sleep(10)  # пока 10, 3мин = 180сек
loop_m('Готово, я сделал для тебя досье', None, config.TOKEN_CRIMINALIST)

# новости 2
sleep(300)  # пока 10, 5мин = 300сек
...
for i in range(3):
    ...
    # помощь
    if i == 2:
        if relationships_Bot__criminalist >= 3:
            loop_m('Я смог отвлечь фдералов, давай быстрей последенего опрашивай', None, config.TOKEN_CRIMINALIST)
        else:
            break
...
if wait() == 0:
    loop_m('Мы задердали ее до конца расследования. Вот её алиби: Я с ней разговаривала по телефону утром, сказала, '
    ...
    loop_m('Мы задержали его, до конца расследования. Его алиби: Да, не спорю, это странно, что я вернулся раньше.'
...
loop_m('Так, ну пока у нас затишье можете взглянуть на всё происходяще и подумать')

sleep(60)  # должно быть 60сек
...
loop_m('Есть один человек, торгует наркотой, вот это сделал он. Я у него какое-то время брал дозу,'
       ' но потом закончил...')
sleep(4)
edit_main('Bot__help', 0, 'Имя вроде бы Стас, рост под 178-180, тусуется всегда в баре по улице Н. Там найдете')
...
if time_relationships == 1:
    ...
    if wait() == 1:
        loop_m('Совкин Станислав Максимович, 25 лет, 15.12.1995 г.р. Нет родных, все близкие мертвы. Был отправлен в '
               'детский дом в возрасте пяти лет. Имел знакомство с курением и проблемами с алкоголем. Стоял долгое '
               'время на учете в милиции за алкоголь и воровство. В 18 лет покинул пределы детского дома, начал '
               'развивать собственную торговлю незаконными веществами. Стал мелким дилером, занимался этим долгое '
               'время, после чего завязал из-за неудачной ситуации. Не имеет образования помимо окончания 9 классов. '
               'Никаких заболеваний в медицинской карте нет. Братьев и сестер нет, семью не видел с детства.', None,
               config.TOKEN_CRIMINALIST)
        sleep(10)
...
sleep(3)
loop_m('Убит мужчина 27 лет в собственной квартире, в районе Н. Предположительная причина смерти: '
       'передозировка инсулином', None, config.TOKEN_POLICE)
sleep(1)
loop_m('Время смерти: 07.07.2020, 19:00.', None, config.TOKEN_POLICE)
sleep(3)
loop_m('Вам надо срочно выезжать, у нас как всегда нет много времени. Вам также скоро скинут досье на жертву',
       None, config.TOKEN_POLICE)

sleep(60)  # должно быть 1мин - 60сек
# досье
loop_m('Досье', None, config.TOKEN_CRIMINALIST)

sleep(180)  # должно быть 3мин = 180сек
# Новости
...
#
# # считает улики и видёт подсчёт времени
# for _ in range(10):  # 900сек = 10 мин
#     f = open('text.txt', 'r+', encoding='utf8')
#     text = [str(i) for i in f.read().split('|')]
#     f.close()
#     if text[0] == 'main':
...
# разговор с подозреваемыми
for i in range(2):
    edit_main('Bot__police', 21, 'Кого берёшь?')  # отдаём через файл команду полицаю
    count = wait()  # выбираем кого опрашиваем
...
sleep(3)
loop_m('Думаю пришло время уже что-то делать. У нас 5 убийство и есть подозреваемые', None, config.TOKEN_POLICE)
loop_m('Предлогаяю сопоств=авить все данные и выбрать убийцу...')
sleep(2)
loop_m('Сейчас тебе криминалист скинет по всем подозреваемым досье\nТы должен подумать. '
       'Скоро я тебе напишу и спрошу что ты думаешь...')

loop_m('Сейчас отправлю досье всех подозреваемых', None, config.TOKEN_CRIMINALIST)
for i in suspect:
    sleep(2)
    loop_m('---')
    if i == 'Подруга, Инна':
