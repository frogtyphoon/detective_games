    ...
    if relationships_Bot__police >= 0:
        ...
        if relationships_Bot__police <= 0:
            if police.bot_police_main(2, 'Совсем страх потерял?') < 0:
                ...
                await bot.send_message(config.ID_PERSON, 'Я сейчас на исходе, поэтому бегом на '
                                                         'место преступления по адресу "УЛИЦА"\nКонец связи')
                time_talk_answer = True
            else:
                relationships_Bot__police += 1
                await bot.send_message(config.ID_PERSON, 'Кратко ввожу в курс дела. Рядом произошёл суицид, сходи да проверь там всё')
